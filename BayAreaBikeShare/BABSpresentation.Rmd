...
```{r}
subType <- trip  %>%  group_by(Subscriber.Type)  %>% summarise(count=n())
```
- 24 hour or 3 day passes can be bought at a station kiosk.
- 1 year subscriptions available online. `r round(100 * subType$count[2]/sum(subType$count)) ` per cent of trips are by subscribers.
...
- The longest trip made lasted `r round(max(trip$Duration)/ ( 60 * 60 *24))` days! ($67153 late fees...)
...
```
- `r round( 100 *SFCT$count[2] / sum(SFCT$count))` per cent of usage here is long term subscribers. There are two stations.
...
ggplot(bikeCountsNew, aes(x=Year, y = value, col = Location))+
  ...
  geom_line()+
  theme(legend.position="none")+
  labs(x="Year", y= "Manual bike count", title = "Bike usage from various locations in SF") +
...
- [TripAdvisor](https://www.tripadvisor.co.uk/Attractions-g60713-Activities-San_Francisco_California.html#MAPVIEW) will give us the tourist hot spots:
...
ggplot(inOut, aes(x = Station, y = diff, col = as.factor(difference)))+
  ...
  labs(x="Station", y= "Bikes deposited -bike hired")+
...
ggmap(BABSmap)+
  ...
  labs(x = "longitude", y = "latitude", 
       size ="Imbalance",col ="Skew",title = "Bike imbalance")+
          ...
          axis.title.y=element_blank(),
          panel.background=element_blank(),panel.border=element_blank(),panel.grid.major=element_blank(),
          panel.grid.minor=element_blank(),plot.background=element_blank())+scale_colour_manual(values = c("green", "red", "blue"))
```
